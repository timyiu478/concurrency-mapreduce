...
typedef struct MapperTasks {
    char **file_name; // The file names to be processed by the mapper
...
void MapperTasks_Free(MapperTasks *tasks) {
    for (int i = 0; i < tasks->num_files; i++) {
        free(tasks->file_name[i]);
    }
    free(tasks->file_name);
...
char *get_mapper_task(MapperTasks *tasks) {
    ...
    if (tasks->index < tasks->num_files) {
        return tasks->file_name[tasks->index++];
    }
    sem_post(&tasks->sem);
...
void KVs_Init(KVs *kvs) {
    kvs->key = NULL;
    kvs->value = malloc(DYNAMIC_ARRAY_CAPACITY * sizeof(char *));
    kvs->size = 0;
...
void KVs_Resize(KVs *kvs) {
    kvs->capacity *= 2;
    kvs->value = realloc(kvs->value, kvs->capacity * sizeof(char *));
}
...
void KVsStore_Insert(KVsStore *store, char *key, char *value) {
    ...
    for (int i = 0; i < store->size; i++) {
        if (store->kvs[i].key && strcmp(store->kvs[i].key, key) == 0) {
            ...
            store->kvs[i].value[store->kvs[i].size] = strdup(value);
            store->kvs[i].size++;
            sem_post(&store->sem);
            return;
        }
    ...
    store->kvs[store->size].value[0] = strdup(value);
...
char *Reducer_Getter(char *key, int partition_number) {
    ...
    if (kvs != NULL && kvs->index < kvs->size) {
        return kvs->value[kvs->index++];
    }
    return NULL; // No more values
}

void MR_Emit(char *key, char *value) {
    unsigned long partition_number = global_partition(key, global_num_partitions);
